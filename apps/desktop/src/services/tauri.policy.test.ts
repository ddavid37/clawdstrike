...
describe("tauri policy helpers", () => {
  ...
  it("throws when Tauri is unavailable", async () => {
    ...
    await expect(policyValidate("version: \"1.2.0\"")).rejects.toThrow("policyValidate requires Tauri");
    await expect(policyEvalEvent({ eventId: "evt-1" })).rejects.toThrow("policyEvalEvent requires Tauri");
  ...
  it("invokes policy bridge commands in Tauri mode", async () => {
    ...
    await expect(policyEvalEvent({ eventId: "evt-1" })).resolves.toMatchObject({ command: "policy_eval" });
    expect(invokeMock).toHaveBeenLastCalledWith("policy_eval_event", { event: { eventId: "evt-1" } });
