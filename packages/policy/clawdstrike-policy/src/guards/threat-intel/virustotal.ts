...
export function createVirusTotalGuard(
  config: VirusTotalConfig,
  asyncCfg: AsyncGuard["config"],
): AsyncGuard {
      ...
      if (event.eventType === "file_write" && event.data.type === "file") {
        ...
        if (detections >= minDetections) {
          return deny(
            "clawdstrike-virustotal",
            "critical",
            `VirusTotal: ${detections} detections for file hash ${hash}`,
            {
              hash,
      ...
      if (event.eventType === "network_egress" && event.data.type === "network") {
        ...
        if (detections >= minDetections) {
          return deny(
            "clawdstrike-virustotal",
            "high",
            `VirusTotal: ${detections} detections for URL ${targetUrl}`,
            {
              url: targetUrl,
              last_analysis_stats: stats,
              status: resp.status,
