...
import { ClaudeCodeToolBoundary } from './tool-boundary.js';
...
export class ClaudeCodeAdapter implements FrameworkAdapter {
  ...
  createContext(metadata: Record<string, unknown> = {}): SecurityContext {
    return createSecurityContext({
      metadata: { framework: 'claude-code', ...metadata },
    });
  }
  ...
  getHooks(): FrameworkHooks {
    return {
      createCallbackHandler: () =>
        new ClaudeCodeToolBoundary({ interceptor: this.interceptor, config: this.config }),
      wrapTool: tool => tool,
